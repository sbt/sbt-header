...
/**
  ...
  * list of filters.
  *
  * Play will automatically use filters from any class called `Filters` that is placed the root
  * package. You can load filters from a different class by adding a `play.http.filters` setting to
  * the `application.conf` configuration file.
  *
  * @param env
  *   Basic environment settings for the current application.
  * @param exampleFilter
